...
enum StoatError: Error {
    struct StoatHTTPError: Codable {
        var type: String
        var location: String
    }
    case Alamofire(AFError)
    case HTTPError(StoatHTTPError?, Int)
...
struct HTTPClient {
    ...
    >(
    ...
    ) async -> Result<DataResponse<String, AFError>, StoatError> {
        ...
        if ![200, 201, 202, 203, 204, 205, 206, 207, 208, 226].contains(code) {
            let errorBody = try? response.value.map { try JSONDecoder().decode(StoatError.StoatHTTPError.self, from: $0.data(using: .utf8)!) }
    ...
    >(
        ...
        encoder: ParameterEncoder = JSONParameterEncoder.default,
        headers: HTTPHeaders? = nil
    ) async -> Result<EmptyResponse, StoatError> {
        return await innerReq(method: method, route: route, parameters: parameters, encoder: encoder, headers: headers).flatMap { response in
            if let error = response.error {
                return .failure(.Alamofire(error))
            } else {
    ...
    func fetchSelf() async -> Result<User, StoatError> {
        await req(method: .get, route: "/users/@me")
    }

    func signout() async -> Result<EmptyResponse, StoatError> {
        await req(method: .post, route: "/auth/session/logout")
    }

    func fetchApiInfo() async -> Result<ApiInfo, StoatError> {
        await req(method: .get, route: "/")
    }

    func sendMessage(channel: String, replies: [ApiReply], content: String, attachments: [(Data, String)], nonce: String) async -> Result<Message, StoatError> {
    ...
    func fetchUser(user: String) async -> Result<User, StoatError> {
        await req(method: .get, route: "/users/\(user)")
    }

    func deleteMessage(channel: String, message: String) async -> Result<EmptyResponse, StoatError> {
        await req(method: .delete, route: "\(baseURL)/channels/\(channel)/messages/\(message)")
    }

    func fetchHistory(channel: String, limit: Int, before: String?) async -> Result<FetchHistory, StoatError> {
    ...
    func fetchMessage(channel: String, message: String) async -> Result<Message, StoatError> {
        await req(method: .get, route: "/channels/\(channel)/messages/\(message)")
    }

    func fetchDms() async -> Result<[Channel], StoatError> {
        await req(method: .get, route: "/users/dms")
    }

    func fetchProfile(user: String) async -> Result<Profile, StoatError> {
        await req(method: .get, route: "/users/\(user)/profile")
    }

    func uploadFile(data: Data, name: String, category: FileCategory) async -> Result<AutumnResponse, StoatError> {
        ...
        return await session.request(
            ...
            .response
            .result
            .mapError(StoatError.Alamofire)
    }

    func fetchSessions() async -> Result<[Types.Session], StoatError> {
        await req(method: .get, route: "/auth/session/all")
    }

    func deleteSession(session: String) async -> Result<EmptyResponse, StoatError> {
        await req(method: .delete, route: "/auth/session/\(session)")
    }

    func joinServer(code: String) async -> Result<JoinResponse, StoatError> {
        await req(method: .post, route: "/invites/\(code)")
    }

    func reportMessage(id: String, reason: ContentReportPayload.ContentReportReason, userContext: String) async -> Result<EmptyResponse, StoatError> {
    ...
    func reportUser(id: String, reason: ContentReportPayload.ContentReportReason, userContext: String) async -> Result<EmptyResponse, StoatError> {
    ...
    func createAccount(email: String, password: String, invite: String?, captcha: String?) async -> Result<EmptyResponse, StoatError> {
    ...
    func createAccount_VerificationCode(code: String) async -> Result<AccountCreateVerifyResponse, StoatError> {
        await req(method: .post, route: "/auth/account/verify/\(code)")
    }

    func createAccount_ResendVerification(email: String, captcha: String?) async -> Result<EmptyResponse, StoatError> {
        await req(method: .post, route: "/auth/account/reverify", parameters: ["email": email, "captcha": captcha])
    }

    func sendResetPasswordEmail(email: String, captcha: String?) async -> Result<EmptyResponse, StoatError> {
        await req(method: .post, route: "/auth/account/reverify", parameters: ["email": email, "captcha": captcha])
    }

    func resetPassword(token: String, password: String, removeSessions: Bool = false) async -> Result<EmptyResponse, StoatError> {
        await req(method: .patch, route: "/auth/account/reset_password", parameters: PasswordResetPayload(token: token, password: password))
    }

    func checkOnboarding() async -> Result<OnboardingStatusResponse, StoatError> {
        await req(method: .get, route: "/onboard/hello")
    }

    func completeOnboarding(username: String) async -> Result<EmptyResponse, StoatError> {
        await req(method: .post, route: "/onboard/complete", parameters: ["username": username])
    }

    func acceptFriendRequest(user: String) async -> Result<User, StoatError> {
        await req(method: .put, route: "/users/\(user)/friend")
    }

    func removeFriend(user: String) async -> Result<User, StoatError> {
        await req(method: .delete, route: "/users/\(user)/friend")
    }

    func blockUser(user: String) async -> Result<User, StoatError> {
        await req(method: .put, route: "/users/\(user)/block")
    }

    func unblockUser(user: String) async -> Result<User, StoatError> {
        await req(method: .delete, route: "/users/\(user)/block")
    }

    func sendFriendRequest(username: String) async -> Result<User, StoatError> {
        await req(method: .post, route: "/users/friend", parameters: ["username": username])
    }

    func openDm(user: String) async -> Result<Channel, StoatError> {
        await req(method: .get, route: "/users/\(user)/dm")
    }

    func fetchUnreads() async -> Result<[Unread], StoatError> {
        await req(method: .get, route: "/sync/unreads")
    }

    func ackMessage(channel: String, message: String) async -> Result<EmptyResponse, StoatError> {
        await req(method: .put, route: "/channels/\(channel)/ack/\(message)")
    }

    func createGroup(name: String, users: [String]) async -> Result<Channel, StoatError> {
        await req(method: .post, route: "/channels/create", parameters: GroupChannelCreate(name: name, users: users))
    }

    func createServer(name: String, description: String?, nsfw: Bool?) async -> Result<CreateServerResponse, StoatError> {
        await req(method: .post, route: "/servers/create", parameters: ServerCreatePayload(name: name, description: description, nsfw: nsfw))
    }

    func createInvite(channel: String) async -> Result<Invite, StoatError> {
        await req(method: .post, route: "/channels/\(channel)/invites")
    }

    func deleteInvite(code: String) async -> Result<EmptyResponse, StoatError> {
        await req(method: .delete, route: "/invites/\(code)")
    }

    func joinVoiceChannel(channel: String, node: String) async -> Result<VoiceChannelToken, StoatError> {
        await req(method: .post, route: "/channels/\(channel)/join_call", parameters: ["node": node])
    }

    func fetchMember(server: String, member: String) async -> Result<Member, StoatError> {
        await req(method: .get, route: "/servers/\(server)/members/\(member)")
    }

    func editServer(server: String, edits: ServerEdit) async -> Result<Server, StoatError> {
        await req(method: .patch, route: "/servers/\(server)", parameters: edits)
    }

    func deleteServer(server: String, leaveSilently: Bool = false) async -> Result<EmptyResponse, StoatError> {
        await req(
            method: .delete,
    ...
    func deleteChannel(channel: String, leaveSilently: Bool = false) async -> Result<EmptyResponse, StoatError> {
        await req(
            method: .delete,
    ...
    func reactMessage(channel: String, message: String, emoji: String) async -> Result<EmptyResponse, StoatError> {
        await req(method: .put, route: "/channels/\(channel)/messages/\(message)/reactions/\(emoji)")
    }

    func unreactMessage(channel: String, message: String, emoji: String) async -> Result<EmptyResponse, StoatError> {
        await req(method: .delete, route: "/channels/\(channel)/messages/\(message)/reactions/\(emoji)")
    }

    // settings stuff
    func fetchAccount() async -> Result<AuthAccount, StoatError> {
        await req(method: .get, route: "/auth/account")
    }

    func fetchMFAStatus() async -> Result<AccountSettingsMFAStatus, StoatError> {
        await req(method: .get, route: "/auth/mfa")
    }

    func submitMFATicket(password: String) async -> Result<MFATicketResponse, StoatError> {
        await req(method: .put, route: "/auth/mfa/ticket", parameters: ["password": password])
    }

    func submitMFATicket(totp: String) async -> Result<MFATicketResponse, StoatError> {
        await req(method: .put, route: "/auth/mfa/ticket", parameters: ["totp_code": totp])
    }

    func submitMFATicket(recoveryCode: String) async -> Result<MFATicketResponse, StoatError> {
        await req(method: .put, route: "/auth/mfa/ticket", parameters: ["recovery_code": recoveryCode])
    }

    func generateRecoveryCodes(mfaToken: String) async -> Result<[String], StoatError> {
    ...
    func getTOTPSecret(mfaToken: String) async -> Result<TOTPSecretResponse, StoatError> {
        let headers = HTTPHeaders(dictionaryLiteral: ("X-Mfa-Ticket", mfaToken))
    ...
    func enableTOTP(mfaToken: String, totp_code: String) async -> Result<EmptyResponse, StoatError> {
        let headers = HTTPHeaders(dictionaryLiteral: ("X-Mfa-Ticket", mfaToken))
        return await req(method: .put, route: "/auth/mfa/totp", parameters: ["totp_code": totp_code], headers: headers)
    }

    func disableTOTP(mfaToken: String) async -> Result<EmptyResponse, StoatError> {
        let headers = HTTPHeaders(dictionaryLiteral: ("X-Mfa-Ticket", mfaToken))
        return await req(method: .delete, route: "/auth/mfa/totp", headers: headers)
    }

    func updateUsername(newName: String, password: String) async -> Result<User, StoatError> {
        await req(method: .patch, route: "/users/@me/username", parameters: ["username": newName, "password": password])
    }

    func updatePassword(newPassword: String, oldPassword: String) async -> Result<EmptyResponse, StoatError> {
        await req(method: .patch, route: "/auth/account/change/password", parameters: ["password": newPassword, "current_password": oldPassword])
    }

    func disableAccount(mfaToken: String) async -> Result<EmptyResponse, StoatError> {
        let headers = HTTPHeaders(dictionaryLiteral: ("X-Mfa-Ticket", mfaToken))
        return await req(method: .post, route: "/auth/account/disable", headers: headers)
    }

    func deleteAccount(mfaToken: String) async -> Result<EmptyResponse, StoatError> {
        let headers = HTTPHeaders(dictionaryLiteral: ("X-Mfa-Ticket", mfaToken))
        return await req(method: .post, route: "/auth/account/delete", headers: headers)
    }

    func editMessage(channel: String, message: String, edits: MessageEdit) async -> Result<Message, StoatError> {
        await req(method: .patch, route: "/channels/\(channel)/messages/\(message)", parameters: edits)
    }

    func uploadNotificationToken(token: String) async -> Result<EmptyResponse, StoatError> {
        await req(method: .post, route: "/push/subscribe", parameters: ["endpoint": "apn", "p256dh": "", "auth": token])
    }

    func revokeNotificationToken() async -> Result<EmptyResponse, StoatError> {
        await req(method: .post, route: "/push/unsubscribe")
    }

    func searchChannel(channel: String, query: String) async -> Result<SearchResponse, StoatError> {
    ...
    func fetchMutuals(user: String) async -> Result<MutualsResponse, StoatError> {
        await req(method: .get, route: "/users/\(user)/mutual")
    }

    func fetchInvite(code: String) async -> Result<InviteInfoResponse, StoatError> {
        await req(method: .get, route: "/invites/\(code)")
    }

    func editRole(server: String, role: String, payload: RoleEditPayload) async -> Result<Role, StoatError> {
        await req(method: .patch, route: "/servers/\(server)/roles/\(role)", parameters: payload)
    }

    func setRolePermissions(server: String, role: String, overwrite: Overwrite) async -> Result<Server, StoatError> {
        await req(method: .put, route: "/servers/\(server)/permissions/\(role)", parameters: ["permissions": ["allow": overwrite.a, "deny": overwrite.d]])
    }

    func deleteRole(server: String, role: String) async -> Result<EmptyResponse, StoatError> {
        await req(method: .delete, route: "/server/\(server)/roles/\(role)")
    }

    func createRole(server: String, name: String) async -> Result<RoleWithId, StoatError> {
        await req(method: .post, route: "/servers/\(server)/roles", parameters: ["name": name])
    }

    func setDefaultRolePermissions(server: String, permissions: Permissions) async -> Result<Server, StoatError> {
        await req(method: .put, route: "/servers/\(server)/permissions/default", parameters: ["permissions": permissions])
    }

    func setDefaultRoleChannelPermissions(channel: String, permissions: Permissions) async -> Result<Channel, StoatError> {
        await req(method: .put, route: "/channels/\(channel)/permissions/default", parameters: ["permissions": permissions])
    }

    func setDefaultRoleChannelPermissions(channel: String, overwrite: Overwrite) async -> Result<Channel, StoatError> {
    ...
    func setRoleChannelPermissions(channel: String, role: String, overwrite: Overwrite) async -> Result<Channel, StoatError> {
        await req(method: .put, route: "/channels/\(channel)/permissions/\(role)", parameters: ["permissions": ["allow": overwrite.a, "deny": overwrite.d]])
    }

    func uploadEmoji(id: String, name: String, parent: EmojiParent, nsfw: Bool) async -> Result<Emoji, StoatError> {
        await req(method: .put, route: "/custom/emoji/\(id)", parameters: CreateEmojiPayload(id: id, name: name, parent: parent, nsfw: nsfw))
    }

    func deleteEmoji(emoji: String) async -> Result<EmptyResponse, StoatError> {
        await req(method: .delete, route: "/custom/emoji/\(emoji)")
    }

    func fetchSettings(keys: [String]) async -> Result<SettingsResponse, StoatError> {
        await req(method: .post, route: "/sync/settings/fetch", parameters: ["keys": keys])
    }

    func setSettings(key: String, value: String) async -> Result<EmptyResponse, StoatError> {
        await req(method: .post, route: "/sync/settings/set", parameters: [key: value.replacing("\"", with: "\\\"")])
    }

    func fetchChannelPins(channel: String) async -> Result<SearchResponse, StoatError> {
    ...
    func pinMessage(channel: String, message: String) async -> Result<EmptyResponse, StoatError> {
        await req(method: .post, route: "/channels/\(channel)/messages/\(message)/pin")
    }

    func unpinMessage(channel: String, message: String) async -> Result<EmptyResponse, StoatError> {
        await req(method: .delete, route: "/channels/\(channel)/messages/\(message)/pin")
    }

    func fetchBots() async -> Result<BotsResponse, StoatError> {
        await req(method: .get, route: "/bots/@me")
    }

    func createBot(username: String) async -> Result<Bot, StoatError> {
        await req(method: .post, route: "/bots/create", parameters: ["name": username])
    }

    func deleteBot(id: String) async -> Result<EmptyResponse, StoatError> {
        await req(method: .delete, route: "/bots/\(id)")
    }

    func editBot(id: String, parameters: EditBotPayload) async -> Result<Bot, StoatError> {
        await req(method: .patch, route: "/bots/\(id)", parameters: parameters)
    }

    func fetchMembers(server: String, excludeOffline: Bool) async -> Result<MembersWithUsers, StoatError> {
        await req(method: .get, route: "/servers/\(server)/members?exclude_offline=\(excludeOffline ? "true" : "false")")
    }

    func fetchInvites(server: String) async -> Result<[Invite], StoatError> {
        await req(method: .get, route: "/servers/\(server)/invites")
    }

    func fetchBans(server: String) async -> Result<BansResponse, StoatError> {
        await req(method: .get, route: "/servers/\(server)/bans")
    }

    func editMember(server: String, user: String, payload: EditMemberPayload) async -> Result<Member, StoatError> {
        await req(method: .patch, route: "/servers/\(server)/members/\(user)", parameters: payload)
    }

    func unbanUser(server: String, user: String) async -> Result<EmptyResponse, StoatError> {
        await req(method: .delete, route: "/servers/\(server)/bans/\(user)")
    }

    func createWebhook(channel: String, body: CreateWebhookPayload) async -> Result<Webhook, StoatError> {
        await req(method: .post, route: "/channels/\(channel)/webhooks", parameters: body)
    }

    func fetchWebhooks(channel: String) async -> Result<[Webhook], StoatError> {
        await req(method: .get, route: "/channels/\(channel)/webhooks")
    }

    func deleteWebhook(webhook: String) async -> Result<EmptyResponse, StoatError> {
